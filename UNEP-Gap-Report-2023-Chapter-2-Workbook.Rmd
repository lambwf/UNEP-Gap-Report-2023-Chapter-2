...
## Substitute NGHGI with latest data
...
gas_trend <- gas_trend %>%
  mutate(CO2_LULUCF=ifelse(year==2022,3.9,CO2_LULUCF))
...
data_nghgi_lulucf_totals <- lulucf_nghgi %>%
  ...
  filter(year<2023) %>%
  group_by(year) %>%
  summarise(CO2_LULUCF_NGHGI=sum(value,na.rm=TRUE)/1e9)

data_nghgi_lulucf_totals <- data_nghgi_lulucf_totals %>% 
  add_row(year=2021,CO2_LULUCF_NGHGI=-1.9)
...
``` {r fig_1_global_trend, echo=FALSE, warning= FALSE, fig.width=7, fig.height=6, include=FALSE, fig.path="Results/",dev=c('png','pdf','svg')}
...
p1 <- gas_trend %>% 
  ...
  geom_text(inherit.aes = FALSE, data=gas_totals %>% filter(year %in% c(1990,2000,2010,2020,2022)),aes(x=ifelse(year==2022,year+0.5,year),y=63,label=paste(round(totals,0),"Gt")),
...
gas_comparison <- left_join(gas_comparison,gas_trend %>% 
                              filter(gas!="CO2_LULUCF") %>% 
                              filter(year==2021) %>% 
                              rename(NGHGI=value) %>% 
                              mutate(gas=as.character(gas)) %>% 
                              mutate(gas=ifelse(gas=="CO2_LULUCF_NGHGI","CO2_LULUCF",gas)),
                            by=c("year","gas"))
...
p2 <- gas_comparison %>% 
  ...
  coord_flip() +
  #### text with the totals
  geom_text(inherit.aes = FALSE, data=gas_comparison %>% group_by(var) %>% summarise(value=sum(value)),aes(x=var,y=60,label=paste(round(value,0),"Gt")),        size=3.5,colour="#252525") +
  
  scale_y_continuous(breaks=c(0,10,20,30,40,50,60),limits=c(-2,64)) +
  scale_x_discrete(labels=c("GHG emissions +\nbookkeeping LULUCF","GHG emissions +\ninventory LULUCF")) +
  scale_fill_manual(values=c("#e78ac3","#fc8d62","#8da0cb","#66c2a5","#a6d854"),
                    labels=c("CO2-LULUCF","F-gases","N2O","CH4","CO2-Fossil")) +
  theme_wl() +
...
data_sectors <- left_join(edgar_ghg,sectors %>% select(-sector_edgar8),by = join_by(code, description))
...
uncertainties = data.frame(gas=c("CO2_Fossil","CO2_LULUCF","CO2_LULUCF_NGHGI","CH4","N2O","Fgas"),uncertainty=c(0.08,0.7,0.7,0.3,0.6,0.3))
...
data_table_levels$gas <- fct_relevel(data_table_levels$gas,
                              "GHG","CO2_Fossil","CO2_LULUCF","CO2_LULUCF_NGHGI","CH4","N2O","Fgas")
...
data_table_trend$gas <- fct_relevel(data_table_trend$gas,
                              "GHG","CO2_Fossil","CO2_LULUCF","CH4","N2O","Fgas")
